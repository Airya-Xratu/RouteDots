...
- 🎨 **Yours to style.** One palette covers every colour — ocean, countries,
  ...
  lean as far apart as you like). City names are typeset with the `labels`
  text style (font, size, weight, spacing, colours, halo), and the **source
  and destination airports** can be styled independently (`airports`: dot
  colour / size, pulse ring, per endpoint). Call `rd.setOptions()` /
  `rd.setColors()` to restyle a live scene without restarting the animation.
...
|                                           |                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `new RouteDots(container, options?)`      | Mounts the globe or the flat world. Options: `theme`, `colors`, `world`, `view`, `autoRotate`, `surface`, `borders`, `route` (colours, lifts, **curve angles**, dashes, draw timing), `cities` (dot + **ripple**), `labels` (city-name **text style**), `airports` (**source / destination** endpoint styling), `plane` (**icon** component, size, colour, timing), `camera3d`, `frameRoute`, `fallback`, `flat`, `land`… |
...
| `rd.setOptions(options)`                  | Apply any option live — dashes, angles, ripple, labels, airports, icon, camera; `rd.setWorld('flat')` switches renderer.                                                                                                                                                                                                                                                                                                  |
